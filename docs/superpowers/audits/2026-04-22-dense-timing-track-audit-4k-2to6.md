...
This is the rerun after enabling the parser/renderer red timing validity gate. The old renderer-only
numeric smoke test was a false positive. With invalid source timing rejected before rendering, the
accepted subset has bounded raw BPM/beat-length extrema, but the overall pre-training gate remains
FAIL because the eligible corpus contains invalid authored red timing sections.

## Result

```text
Render Numerics: PASS
Input Timing Validity: FAIL
Overall Gate: FAIL
```

Gate failure is intentional: the renderer must only consume semantically valid red timing points.
The valid subset rendered without NaN/Inf, but 70 eligible maps were rejected by the input timing
validity gate.
...
- SHA-256: `20b2944e4739d0fd197372629f81b74e871704902e50ea9de43fcf17aa4c90a1`
- Schema version: `1`
...
code_commit 1d803bb4cb9dc21b21e3378b6c8f04ff3fd757cb
code_dirty false
...
- Treat the 70 affected maps as excluded from this oracle dense timing track unless a separate design
  decision defines how to model gimmick/SV red timing. They are syntactically red in `.osu`, but they
  are not valid BPM conditioning for `timing_track_20ms_v1`.
