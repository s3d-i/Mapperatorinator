...
DEFAULT_TIMESERIES_PATH = Path(
    "train/artifacts/features/control_v2_timeseries_4k_no_timing_anomalies_2to6.parquet"
)
DEFAULT_SUMMARY_PATH = Path(
    "train/artifacts/features/control_v2_map_summary_4k_no_timing_anomalies_2to6.parquet"
)
DEFAULT_METADATA_PATH = Path(
    "train/artifacts/features/control_v2_artifact_metadata_4k_no_timing_anomalies_2to6.json"
)
...
def build_control_v2_artifacts(
    *,
    ...
    metadata = {
        "schema_version": 1,
        "artifact_name": "control_v2_4k_no_timing_anomalies_2to6",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Stage 1 Control Feature V2 parquet artifacts.")
    ...
    summary_df, metadata = build_control_v2_artifacts(
...
def _load_index_with_source_index(index_path: Path, source_index_path: Path) -> pd.DataFrame:
    ...
    if source_index_path.exists():
        ...
        source_df = source_df[(source_df["difficulty"] >= 2.0) & (source_df["difficulty"] <= 6.0)].reset_index(drop=True)
        if len(source_df) == len(index_df) and source_df["beatmap_id"].astype(str).tolist() == index_df["beatmap_id"].astype(str).tolist():
